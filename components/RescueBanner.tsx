...
export function RescueBanner({ tabId }: Props) {
  ...
  return (
    <div className="mx-4 mt-3 rounded-lg border border-amber-200/80 bg-amber-50/80 overflow-hidden shadow-soft animate-slide-up">
        ...
        <div className="flex-1 min-w-0">
          ...
          {state === 'idle' && failedUrls.length > 0 && (
            <span className="text-sm text-amber-700">
              发现 <strong>{failedUrls.length}</strong> 个来源导入失败
            </span>
          )}
          {state === 'rescuing' && (
            <span className="text-sm text-amber-700 flex items-center gap-1.5">
          ...
          {state === 'done' && (
            <span className="text-sm text-amber-700">
              抢救完成：成功 {successCount}，失败 {failCount}
            </span>
          )}
        </div>
      ...
      {/* Expandable details */}
      {expanded && (
        <div className="border-t border-amber-200 max-h-32 overflow-y-auto">
          {results.length > 0 ? (
            results.map((r, i) => (
              <div key={i} className="px-3 py-1.5 flex items-center gap-2 text-xs border-b border-amber-100 last:border-b-0">
                ...
                <span className="flex-1 truncate text-gray-600" title={r.url}>
                  {r.title || r.url}
                </span>
            ...
            failedUrls.map((url, i) => (
              <div key={i} className="px-3 py-1.5 text-xs text-gray-600 truncate border-b border-amber-100 last:border-b-0" title={url}>
                {url}
              </div>
            ))
          )}
