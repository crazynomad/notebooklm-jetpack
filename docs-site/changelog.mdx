...
- 🐛 **修复播客下载文件名变成无意义 ID**：部分单集（尤其是经 Apple Podcasts 进入、但音频实际托管在小宇宙 CDN 的）下载后文件名变成一串对象 ID，而非按单集友好命名。根因是音频服务器返回的 `Content-Disposition` 头会覆盖 `chrome.downloads.download` 里我们设定的文件名。现在改用 `chrome.downloads.onDeterminingFilename` 强制使用「播客名 / NNN - 单集标题」，优先级高于响应头。（issue #57，已线上验证）
- ✨ **播客单集页打开即自动查询**：停在单集链接（Apple `?i=` / 小宇宙 `/episode/`）时打开 Popup 会自动执行查询、直接列出该集，省去多余的一次「查询」点击，和 YouTube 标签页体验一致；打开整档节目链接仍保留手动查询，避免一开就拉取整列。下载动作仍需你手动确认

## v1.7.9 — 2026-07-03

- 🐛 **修复 X 普通推文导入标题为空**：X 是动态页面，导航后 `document.title` 常常是空的（`og:title` 也拿不到），导致普通推文/推文串导入 NotebookLM 后标题为空、兜底成 `x.com`。现在改用「作者名：首条推文片段」作为标题（长文 Article 仍走文章标题）。已用真实帖子线上验证
- 🐛 **修复导入偶发「假失败」与 `[object Object]` 错误日志**：导入前先探活 NotebookLM 页的 content script，只在没有响应时才注入，避免重复注入产生重复监听器、造成「已成功却报失败」的竞态；错误日志也改为打印真实原因而非 `[object Object]`

## v1.7.8 — 2026-07-03

- 🙋 **X 导入改为「预填不自动导入」**：之前停在 X 帖子页打开 Popup 会立即导入，等于"打开即产生副作用"，用户既没机会决定要不要导、也来不及切换目标 Notebook。现在只自动预填链接，由你选好 Notebook 后手动点「导入」——是否导入、导入到哪里都交回你掌控
- 🧭 **新手引导补上 X 步骤**，欢迎页新增 X（推特）功能卡片，商店文案同步为「帖子/长文·专属标签页」

## v1.7.7 — 2026-07-03

- ✨ **新增独立的 X（推特）导入标签页**：过去导入 X 帖子要先普通导入一次、再到 NotebookLM 页面点「修复」横幅补救，藏得很深。现在 Popup 里有专属 **X** 标签页，粘贴帖子/推文串（`/status/`）或长文 Article（`/article/`）链接即可一步导入——扩展在后台打开页面、等 SPA 渲染后提取正文再导入。停在 X 帖子页时自动识别并预填链接
- 🛡️ **脆弱 DOM 选择器集中管理 + 每周自动巡检**：X/Twitter 的 `data-testid`、NotebookLM 的 Material 类名等最易随第三方改版而失效的选择器，统一收进 `lib/selectors.ts` 单一来源，配 HTML fixture 回归测试与每周云端巡检——选择器一旦失效更早发现、更早修
